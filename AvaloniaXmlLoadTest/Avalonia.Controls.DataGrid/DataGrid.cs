...
namespace Avalonia.Controls
{
    ...
    public partial class DataGrid : TemplatedControl
    {
        ...
        /// <summary>
        /// При выставлении события о нажатии на строку не происходит.
        /// В том числе не просиходит выделения строки.
        /// <remarks>Это нужно так как иначе в определенных кейсах срабатывает выделение строки,
        /// когда пользователь нажал на контрол внутри конетнта ячейки.</remarks>
        /// </summary>
        public bool CurentRowAutoBehaviourDisabled
        {
            get { return GetValue(CurentRowAutoBehaviourDisabledProperty); }
            set { SetValue(CurentRowAutoBehaviourDisabledProperty, value); }
        }
        ...
        /// <remarks>Это нужно так как иначе в определенных кейсах срабатывает выделение строки,
        /// когда пользователь нажал на контрол внутри конетнта ячейки.</remarks>
        public static readonly StyledProperty<bool> CurentRowAutoBehaviourDisabledProperty =
            AvaloniaProperty.Register<DataGrid, bool>(nameof(CurentRowAutoBehaviourDisabled));
        ...
        private void OnItemsPropertyChanged(AvaloniaPropertyChangedEventArgs e)
        {
            if (!_areHandlersSuspended)
            {
                ...
                {
                    DataConnection.DataSource = DataGridDataConnection.CreateView(newItemsSource,this);
                }
                else
                {
            ...
            if (!_areHandlersSuspended)
            {
                ...
                else
                {
                    ...
                    if (slot != CurrentSlot)
                    {
                        ...
                        if ((slot >= SlotCount || slot < -1) && DataConnection.CollectionView != null)
                        {
        ...
        private void OnHeadersVisibilityChanged(AvaloniaPropertyChangedEventArgs e)
        {
            ...
            // Columns
            if (newValueCols != oldValueCols && _columnHeadersPresenter != null)
            {
            ...
            // Rows
            if (newValueRows != oldValueRows && _rowsPresenter != null)
            {
                foreach (var element in _rowsPresenter.Children)
                {
        ...
        /// <summary>
        /// Occurs when cell is double tapped.
        /// </summary>
        ...
        /// </summary>
        internal virtual void OnCellDoubleTapped(DataGridCellDoubleTappedEventArgs e)
        {
            CellDoubleTapped?.Invoke(this, e);
        }
        ...
        protected override void OnTemplateApplied(TemplateAppliedEventArgs e)
        {
            ...
            if (_columnHeadersPresenter != null)
            {
                ...
                foreach (DataGridColumn column in ColumnsItemsInternal.OrderBy(x => x.DisplayIndex))
                {
        ...
        internal void ProcessSelectionAndCurrency(int columnIndex, object item, int backupSlot, DataGridSelectionAction action, bool scrollIntoView)
        {
            ...
            try
            {
                ...
                if (CurrentSlot != slot || (CurrentColumnIndex != columnIndex && columnIndex != -1))
                {
                    ...
                    if (columnIndex != -1 && (!SetCurrentCellCore(
                                columnIndex, slot,
                                commitEdit: true,
        ...
        internal bool ScrollSlotIntoView(int columnIndex, int slot, bool forCurrentCellChange, bool forceHorizontalScroll)
        {
            ...
            if (CurrentColumnIndex >= 0 &&
                ...
                (!CommitEditForOperation(columnIndex, slot, forCurrentCellChange) || IsInnerCellOutOfBounds(columnIndex, slot)))
            {
                return false;
            }
        ...
        private void DataGrid_GotFocus(object sender, RoutedEventArgs e)
        {
            if (!ContainsFocus)
            {
                ContainsFocus = true;
                ApplyDisplayedRowsState(DisplayData.FirstScrollingSlot, DisplayData.LastScrollingSlot);
                if (CurrentColumnIndex != -1 && IsSlotVisible(CurrentSlot) && DisplayData.GetDisplayedElement(CurrentSlot) is DataGridRow row)
                {
        ...
        private void DataGrid_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (ContainsFocus)
            {
                ...
                while (focusedObject != null)
                {
                    ...
                    if (parent == null && focusedObject is Control element)
                    {
                ...
                if (focusLeftDataGrid)
                {
                    ...
                    if (CurrentColumnIndex != -1 && IsSlotVisible(CurrentSlot) &&
                        DisplayData.GetDisplayedElement(CurrentSlot) is DataGridRow row)
                    {
                        row.Cells[CurrentColumnIndex].UpdatePseudoClasses();
                    }
                }
                else if (!dataGridWillReceiveRoutedEvent && focusedObject is Control focusedElement)
                {
        ...
        private bool EndCellEdit(DataGridEditAction editAction, bool exitEditingMode, bool keepFocus, bool raiseEvents)
        {
            ...
            if (!(editingCell.Content is Control editingElement))
            {
                return false;
            }
            if (raiseEvents)
