    ...
    internal class DataGridColumnCollection : ObservableCollection<DataGridColumn>
    {
        ...
        protected override void InsertItem(int index, DataGridColumn item)
        {
            try
            {
                ...
                if (item == null)
                {
                    throw new ArgumentNullException(nameof(item), "dataGridColumn");
                }

                int columnIndexWithFiller = index;
                if (item != RowGroupSpacerColumn && RowGroupSpacerColumn.IsRepresented)
                {
                    columnIndexWithFiller++;
                }

                // get the new current cell coordinates
                DataGridCellCoordinates newCurrentCellCoordinates = _owningGrid.OnInsertingColumn(index, item);

                // insert the column into our internal list
                ItemsInternal.Insert(columnIndexWithFiller, item);
                item.Index = columnIndexWithFiller;
                item.OwningGrid = _owningGrid;
                item.RemoveEditingElement();
                if (item.IsVisible)
                {
                ...
                _owningGrid.OnInsertedColumn_PreNotification(item);
                _owningGrid.OnColumnCollectionChanged_PreNotification(true /*columnsGrew*/);

                if (item != RowGroupSpacerColumn)
                {
                    base.InsertItem(index, item);
                }
                _owningGrid.OnInsertedColumn_PostNotification(newCurrentCellCoordinates, item.DisplayIndex);
        ...
        {
            RemoveItemPrivate(index, false /*isSpacer*/);
        }
