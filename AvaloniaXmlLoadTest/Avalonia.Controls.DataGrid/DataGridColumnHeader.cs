...
namespace Avalonia.Controls
{
    ...
    public class DataGridColumnHeader : ContentControl
    {
        ...
        {
            // Parma Fix:	разделитель колонок в шапке.
            _desiredSeparatorVisibility = true;
            // Parma Fix
        ...
        internal void ProcessSort(InputModifiers inputModifiers)
        {
                ...
                && OwningGrid.DataConnection.SortDescriptions != null)
            {
                ...
                using (collectionView.DeferRefresh())
                {
                    ...
                    if (sort != null)
                    {
						// После сортировки по убывания, сбрасываем сортиорвку.
                        newSort = !sort.Descending ? sort.SwitchSortDirection() : null;
        ...
        internal void OnMouseLeftButtonDown(ref bool handled, PointerEventArgs args, Point mousePosition)
        {
            ...
            if (OwningGrid != null && OwningGrid.ColumnHeaders != null)
            {
                ...
                {
                    _dragStart = this.Translate(OwningGrid.ColumnHeaders, mousePosition);
        ...
        private void OnMouseMove_BeginReorder(Point mousePosition)
        {
            DataGridColumnHeader dragIndicator = new DataGridColumnHeader
            {
                ...
                IsEnabled = false,
                // Parma Fix: падение при изменении порядка колонок мышкой, в которых контент контрол, а не текст.
                Content = new Rectangle() { Width = Width, Height = Height, Fill = Brushes.Gray },
                //Content = content,
                // Parma Fix
